...
class UrlaubskasseService {
  ...
  Future<List<Expense>> fetchExpenses(String tripId) async {
    final expRows = await _db
        ...
        .select(
            'id, trip_id, spent_by, amount, description, expense_date, split_method, category, created_at')
        .eq('trip_id', tripId)
  ...
  Future<void> addBooking({
    ...
    final inserted = await _db
        ...
        .select('id')
        .single();
    await _insertBookingParticipants(inserted['id'] as String, participantIds);
  }
  ...
  Future<void> updateBooking({
    ...
    await _db.from('urlaub_bookings').update({
      'booked_by': bookedBy,
      'title': title,
      'amount': amount,
      'booking_date': bookingDate,
    ...
    await _db.from('urlaub_booking_participants').delete().eq('booking_id', id);
    await _insertBookingParticipants(id, participantIds);
  }

  Future<void> deleteBooking(String id) async {
    await _db.from('urlaub_booking_participants').delete().eq('booking_id', id);
    await _db.from('urlaub_bookings').delete().eq('id', id);
  }

  Future<void> _insertBookingParticipants(
      String bookingId, List<String> personIds) async {
    final rows = personIds
        .map((pid) => {'booking_id': bookingId, 'person_id': pid})
        .toList();
    if (rows.isNotEmpty) {
      await _db.from('urlaub_booking_participants').insert(rows);
    }
  }
